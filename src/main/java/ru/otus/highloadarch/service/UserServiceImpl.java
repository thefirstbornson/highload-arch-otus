...
public class UserServiceImpl implements UserService {
    ...
    public List<User> generateUsers(Long count) {
        ...
        for(int i=1; i<=count; i++){
            ...
            user.setSex(Arrays.asList("м","ж").get(new Random().nextInt(2)));
            user.setInterests(Arrays.asList("java","c++","docker").get(new Random().nextInt(3)));
            user.setPassword("default");
            user.setEMail(DigestUtils.md5DigestAsHex(user.getLastName().getBytes())+"@otus.ru");
